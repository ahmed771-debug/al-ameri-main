...
const UserPrograms = () => {
  ...
  const videoContent = useMemo(() => {
    ...
    if (subscription.length === 0) {
      if (trainers && trainers && trainers[0]?.program?.video?.file) {
        console.log(trainers[0]?.program?.video?.file, "ssss");
        return (
          <video
            className="mb-2"
            controls
    ...
    <div className="text-center mb-4">
      ...
      <div className="border-spacing-0 mx-auto max-w-[80rem] sss box-border">
        {trainers.map((item: any, index: any) => {
          ...
          const isDisabled =
            (!isAuthenticated || subscription.length === 0) && index > 0;
          return (
            <div
              className={`${
                isDisabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"
              }  flex justify-between p-4 md:px-16 sm:px-0 border border-gray-200 mb-2 rounded-lg transition-colors duration-300 ease-in-out  hover:bg-gray-200`}
              key={index}
            >
              <div
                onClick={() => {
                  if (subscription.length !== 0 || index === 0) {
